...
function anneal(path::QPath, ℓρ_0, ℓρ_T, t)
    ...
    if q == 1
        ...
        ℓβ = log(β)
        1/(1 - q)*logaddexp((1 - q)*ℓρ_0 + log1mexp(ℓβ), (1 - q)*ℓρ_T + ℓβ)
    end
end
...
struct AnnealedJumpProposal{Prop,AnnealPath<:AbstractAnnealingPath} <: AbstractJumpProposal
    local_proposal :: Prop
    path           :: AnnealPath
end

struct AnnealedTarget{Model,MF,MB,LF,LB,AnnealPath<:AbstractAnnealingPath}
    model           :: Model
    t               :: Int
    map_fwd         :: MF
    map_bwd         :: MB
