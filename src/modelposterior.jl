...
function modelposterior(
    ...
    order_counts    = Dict([(i, count(==(i), order_posterior)) for i in unique(order_posterior)])

    acc_rate_estimates    = estimate_conditional_acceptance_rate(jump_stats)
    log_bayes_factors_adj = estimate_bayes_factors_raoblackwellized(acc_rate_estimates, order_counts, k_max, max_log_bayesfactor)
    ...
    log_bayes_factors_0k = vcat([log_bayes_factor_00], cumsum(log_bayes_factors_adj))
    ...
    sup = 0:(k_max + 1)
    DiscreteNonParametric{eltype(sup),eltype(model_prob),typeof(sup),typeof(model_prob)}(
        sup, model_prob
    )
end
