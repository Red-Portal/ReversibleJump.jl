...
function ReversibleJumpMCMC(
    ...
    ReversibleJumpMCMC(jump_proposal, move_pairs, move_weights, order_kernel, mcmc_kernel)
end
...
function ReversibleJumpMCMC(
    ...
    ReversibleJumpMCMC(jump_proposal, move_pairs, move_prob, order_kernel, mcmc_kernel)
end
...
function AbstractMCMC.step(
    ...
    initial_params,
    RJState(initial_params, logdensity(model, initial_params), initial_order, NamedTuple())
end
...
function AbstractMCMC.step(
    ...
    next = if rand(rng, Bernoulli(p_update))
        ...
        transition_jump(rng, move, jump_proposal, prev, mcmc_kernel, model, order_kernel)
    end
