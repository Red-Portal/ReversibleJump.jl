...
@testset "modelposterior" begin
    ...
    model = DiscreteProductModel(truncated(Poisson(5); lower=1, upper=10), prior)
    ...
    rjmcmc          = ReversibleJump.ReversibleJumpMCMC(prior, AnnealedJumpProposal(prop, path), mcmc)
    samples, stats  = ReversibleJump.sample(rjmcmc, model, n_samples, initial_order, initial_params, show_progress=false)
