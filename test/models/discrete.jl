...
struct DiscreteProductModel{
    C1<:Distributions.DiscreteDistribution,C2<:Distributions.DiscreteDistribution
...
function ReversibleJump.logdensity(model::DiscreteProductModel, θ)
    logpdf(model.order_dist1, length(θ)) + logpdf(model.order_dist2, length(θ))
end

function posterior(model::DiscreteProductModel{C1,C2}) where {C1<:Truncated,C2<:Truncated}
...
function ReversibleJump.local_proposal_logpdf(
    ::Union{DiscreteModel,DiscreteProductModel}, ::ConstantLocalProposal, θ, j
)
    0.0
end

function ReversibleJump.local_insert(::Union{DiscreteModel,DiscreteProductModel}, θ, j, θj)
    insert!(copy(θ), j, θj)
end

function ReversibleJump.local_deleteat(::Union{DiscreteModel,DiscreteProductModel}, θ, j)
