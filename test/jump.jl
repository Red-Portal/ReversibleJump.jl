...
@testset "jump with perfect mcmc transitions" begin
    ...
    @testset for jump_proposal in
                 [AnnealedJumpProposal(prop, GeometricPath(4)), IndepJumpProposal(prop)]
        @testset "birth" begin
            ...
            @assert haskey(state′.stats, :proposal_order) &&
                state′.stats.proposal_order == k_init + 1
            @assert haskey(state′.stats, :previous_order) &&
                state′.stats.previous_order == k_init
